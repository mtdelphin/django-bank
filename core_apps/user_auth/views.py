        ...
        try:
            ...
            if user:
                ...
                if failed_attempts >= settings.LOGIN_ATTEMPTS:
                    ...
                    logger.error(
                        f"Failed login atttempt  for non-existent user: {email}"
                    )
        ...
        if refresh_res.status_code == status.HTTP_200_OK:
            ...
            if access_token and refresh_token:
                ...
                refresh_res.data["message"] = (
                    "Access or refresh not found in refresh response data"
                )
                logger.error("Access or refresh not found in refresh response data")
...
class OTPVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if user.is_locked_out:
            return Response(
                {
                    "error": f"Account is locked due to multiple login failed attempts"
...
class LogoutAPIView(APIView):
    def post(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        logger.info("POST request logout hit")
