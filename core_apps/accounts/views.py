...
class InitiateWithdrawalView(generics.CreateAPIView):
    ...
    def create(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            if not account.fully_activated and not account.kyc_verified:
                return Response(
                    {
                        "error": "Your Account is not fully verified. Please complete "
            ...
            return Response(
                {"error": "Your are not authorize to withdraw from this account"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        return Response(
            {
                "message": "Withdrawal Initiated. Please verify your username "
...
class VerifyUsernameAndWithdrawView(generics.CreateAPIView):
    ...
    def create(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        try:
            ...
            return Response(
                {"error": f"Account number {account_number} doest not exist"},
                status=status.HTTP_404_NOT_FOUND,
            )
        if account.account_balance < amount:
            return Response(
