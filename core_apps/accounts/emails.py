...
def send_account_creation_email(user, bank_account):
    ...
    try:
        email.send()
        logger.info(f"Account Created email sent successfully to: {user.email}")
...
def send_full_activation_email(account: BankAccount) -> None:
    ...
    try:
        email.send()
        logger.info(
            f"Account Fully Activated email sent successfully to: {account.user.email}"
        )
    except Exception as e:
        logger.error(
...
def send_deposit_email(
    ...
    try:
        email.send()
        logger.info(f"Deposit confirmation email sent successfully to: {user_email}")
    except Exception as e:
        logger.error(
...
def send_withdrawal_email(
    ...
    html_email = render_to_string("emails/withdrawal_notification.html", context)
    plain_email = strip_tags(html_email)
    email = EmailMultiAlternatives(subject, plain_email, from_email, recipient_list)
    email.attach_alternative(html_email, "text/html")
    ...
    try:
        email.send()
        logger.info(f"Withdrawal confirmation email sent successfully to: {user_email}")
    except Exception as e:
        logger.error(
...
def send_transfer_email(
    ...
    try:
        ...
        logger.info(
            f"Transfer notification email sent to sender to: {sender_email} "
            f"and receiver {receiver_email}"
        )
    except Exception as e:
        logger.error(f"Failed to send transfer notification emails. Error: {str(e)}")


def send_tranfer_otp_email(email, otp) -> None:
