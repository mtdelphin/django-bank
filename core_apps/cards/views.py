...
class VirtualCardDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"message": "Card successfully destroyed"}, status=status.HTTP_200_OK
            )
...
class VirtualCardTopUpAPIView(generics.UpdateAPIView):
    ...
    def get_object(self) -> VirtualCard:
        obj = super().get_object()
        return obj
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"error": "The amount {amount} is not valid"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if amount <= 0:
            return Response(
                {"error": "Amount must be greater than zeo"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        logger.info(
            ...
            f"{virtual_card.user.full_name}.Transaction ID: {card_transaction.id}"
        )
